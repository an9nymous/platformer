export class Axes{
    ...
    constructor(positions, swingDuration){
            ...
            this.axes.push(
                add([
                    ...
                    area({
                        ...
                        collisionIgnore: ["spiders", "flame"], //?ignora questi gameogj
                    }),
                    pos(position),
                    scale(4),
                    anchor(vec2(0, -0.75)),
    ...
    async swing(axe, angle, swingDuration) {
    ...
    setMovementPattern() {
        for (const [index, axe] of this.axes.entries()) {
            const swingLeft = axe.onStateEnter("swing-left", async () => {
                await this.swing(axe, 90, this.swingDuration[index])
